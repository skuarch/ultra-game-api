...
export class GameValidatorService {
  ...
  throwErrorIfIdIsValid(id: string) {
    if (id) {
  ...
  async throwErrorIfDocumentExists(id: string): Promise<void> {
    if (id) {
      ...
      if (gameDocument) {
        throw new HttpException(`gameDocument ${id} exists`, HttpStatus.FOUND);
      }
    }
  }
  ...
  async throwErrorIfDocumentDoesntExists(id: string): Promise<void> {
    if (id) {
      const gameDocument = await this.gameRepository.getById(id);
      if (!gameDocument) {
        throw new HttpException(`gameDocument ${id} doesn't exists`, HttpStatus.NOT_FOUND);
      }
    }
  }
